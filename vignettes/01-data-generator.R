...
get_country_table_new <- function(n_total = 5 * 10^6, n_countries = 10L) {
...
get_country_data <- function(country_id, n_obs, n_years, urban_range, total_pop) {
  ...
  data.table(
    # country = country_id,
    year = sample(years_range, n_obs, replace = TRUE),
...
dta_full <- 
  get_country_table_new(250*10^6, 150)[, {
  get_country_data(country, n_obs, n_years, urban_range, total_pop)
}, by = .(country)] |> 
  arrange(country, year, welfare)

dta_full |> group_by(country) |> write_dataset("data/sample_250m")
